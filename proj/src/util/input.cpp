...
bool Input::Initialize(HWND hwnd) {
	...
	if (FAILED(result)) {
		MessageBox(NULL, L"failed creating direct input.", L"Input.cpp", MB_OK);
		return false;
	}

	result = m_dinput->CreateDevice(GUID_SysKeyboard, m_keyDevice.ReleaseAndGetAddressOf(), NULL);
	if (FAILED(result)) {
		MessageBox(NULL, L"failed creating key device.", L"Input.cpp", MB_OK);
		return false;
	}

	result = m_keyDevice->SetDataFormat(&c_dfDIKeyboard);
	if (FAILED(result)) {
		MessageBox(NULL, L"failed setting data format.", L"Input.cpp", MB_OK);
		return false;
	}

	result = m_keyDevice->SetCooperativeLevel(hwnd, DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
	if (FAILED(result)) {
		MessageBox(NULL, L"failed setting cooprative level.", L"Input.cpp", MB_OK);
		return false;
	}

	result = m_dinput->CreateDevice(GUID_SysMouse, m_mouseDevice.ReleaseAndGetAddressOf(), NULL);
	if (FAILED(result)) {
		MessageBox(NULL, L"failed creating mouse device.", L"Input.cpp", MB_OK);
		return false;
	}
	...
	if (FAILED(result)) {
		MessageBox(NULL, L"failed setting cooperative level.", L"Input.cpp", MB_OK);
		return false;
	}
	...
	if (FAILED(result)) {
		MessageBox(NULL, L"failed setting property.", L"Input.cpp", MB_OK);
		return false;
	}
